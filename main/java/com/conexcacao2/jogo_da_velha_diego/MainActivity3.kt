...
class MainActivity3 : AppCompatActivity() {
    ...
    fun realizarJogadaEstrategica() {
        // Verificar se há alguma posição onde o jogador humano pode vencer na próxima jogada
        var MELHOR_JOGADA: Pair<Int, Int>? = null
        ...
        if (diagonal2.count { it == "X" } == 2 && diagonal2.count { it == "" } == 1) {
            ...
            MELHOR_JOGADA = Pair(i, 2 - i)
        }

        // Se encontrou uma posição para bloquear, faz a jogada
        if (MELHOR_JOGADA != null) {
            val (x, y) = MELHOR_JOGADA
            tabuleiro[x][y] = "O"
            atualizarTabuleiro(x, y)
        } else {
            // Se não encontrou uma posição para bloquear, joga aleatoriamente
            jogadaAleatoria()
        }
    }
